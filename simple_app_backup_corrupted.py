...
def identify_fixture_opportunities(fixtures_df: pd.DataFrame, threshold: float = 2.5) -> Dict:
    ...
    opportunities['best_attack'] = list(attack_teams[attack_teams <= threshold].sort_values().head(5).index)
    # Best defensive fixtures
    defense_teams = fixtures_df.groupby(['team_id', 'team_name'])['defense_fdr'].mean()
    opportunities['best_defense'] = list(defense_teams[defense_teams <= threshold].sort_values().head(5).index)
    # Best combined fixtures
    combined_teams = fixtures_df.groupby(['team_id', 'team_name'])['combined_fdr'].mean()
    opportunities['best_combined'] = list(combined_teams[combined_teams <= threshold].sort_values().head(5).index)
    # Worst fixtures to avoid
    opportunities['worst_fixtures'] = list(combined_teams[combined_teams >= 4.0].sort_values(ascending=False).head(5).index)
...
class FixtureDataLoader:
    ...
    def process_fixtures_data(self) -> pd.DataFrame:
        ...
        if unfinished_fixtures:
            # Fix the current gameweek calculation
            current_gw_values = [f.get('event') for f in unfinished_fixtures if f.get('event') is not None]
            if current_gw_values:
                current_gw = min(current_gw_values)
            else:
                current_gw = 1
...
class FDRAnalyzer:
    ...
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        self.fdr_colors = {
            1: '#0072B2',  # Blue (for Very Easy)
            2: '#009E73',  # Green (for Easy)
            3: '#F0E442',  # Yellow (for Average)
            4: '#E69F00',  # Orange (for Hard)
            5: '#D55E00'   # Reddish-Orange (for Very Hard)
        }
        self.fdr_labels = {
...
class FPLAnalyticsApp:
    ...
    def _create_enhanced_team_summary(self, fixtures_df, sort_by, ascending_sort):
        ...
        # Create aggregation dictionary with proper types
        agg_dict = {}
        for col in fdr_cols:
