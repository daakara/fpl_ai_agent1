...
from fpl_myteam import load_my_fpl_team  # Import the function to load the FPL team
...
def main():
    ...
    with tab2:
        ...
        if st.button("Load Team"):
            if team_id:
                try:
                    team_data = load_my_fpl_team(int(team_id))  # Fetch the team data
                    st.success("Team loaded successfully!")
                    st.json(team_data)  # Display the team data
